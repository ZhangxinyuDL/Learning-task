...
class ConvLSTM(nn.Module):
    ...
    Parameters:
        ...
        batch_first: Whether or not dimension 0 is the batch or not：：代表在输入数据的五维张量中是否应该把batch放在最开始，如果该值为True，那么代表当前的输入数据batch在最开头，则不用变换，否则需要变换。该参数的作用是确保在ConvLSTM中，batch能够位于tensor的第0维
        bias: Bias or no bias in Convolution
        return_all_layers: Return the list of computations for all layers：选择是返回所有的隐藏层状态和输出，还是返回最后一层的隐藏层状态和输出
...
class EncoderDecoderConvLSTM(nn.Module):
    def __init__(self, nf, in_chan):  # nf:隐藏层维度，inchannel
...
def draw_prediction():
    ...
    mae_error = nn.L1Loss()
    mse_error = nn.MSELoss()  # 交叉熵
    y_tensor = y.unsqueeze(1).to(device) # B, C, T, H, W
    mae_loss = mae_error(output, y_tensor)
    mse_loss = mse_error(output, y_tensor)
...
if __name__ == '__main__':
    ...
    data = data / 255 #数据归一化，类似transform里的totensor
